...
function DiffCell({
  ...
  return (
    <div className={`text-xs ${isRounding ? "font-semibold text-slate-600" : "font-black text-amber-800"}`}>
      {label}: {diff > 0 ? "+" : ""}
      {formatMoney(diff, "")}
    </div>
  );
  ...
  return (
    <div
      className={`shrink-0 border-b px-3 py-2 ${
        rounding ? "border-slate-200 bg-slate-50" : "border-amber-300 bg-amber-50"
      }`}
    >
      <button
        type="button"
      ...
      >
        <span className={`text-xs font-black ${rounding ? "text-slate-700" : "text-amber-900"}`}>{message}</span>
  ...
  return (
    <div className="shrink-0 border-t border-slate-200 bg-slate-50">
      ...
      >
        ...
        {diffSummary ? (
          <span
            className={`text-[10px] font-black ${rounding ? "text-slate-600" : summary.hasMajorMismatch ? "text-amber-800" : "text-slate-500"}`}
          >
          ...
          {summary.hasTotalsDifference ? (
            <div
              className={`mt-3 rounded-lg border px-3 py-2 ${
                rounding ? "border-slate-200 bg-white" : "border-amber-200 bg-amber-50"
              }`}
            >
