      ...
      <div className="relative overflow-hidden rounded-[2.4rem] border border-white/70 bg-gradient-to-br from-orange-50 via-rose-50 to-violet-50 p-0 shadow-[0_28px_90px_rgba(76,29,149,0.13)]">
        ...
        <div className="relative grid min-h-[350px] gap-8 p-7 md:p-9 xl:grid-cols-[1fr_0.95fr] xl:items-center">
                ...
                {outcomes.slice(0, 4).map((outcome, index) => {
                  ...
                  return (
                    <div key={outcome} className="group rounded-[1.5rem] border border-white/80 bg-white/72 p-4 shadow-[0_18px_45px_rgba(76,29,149,0.08)] backdrop-blur-xl">
                      <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br from-violet-100 via-white to-orange-100 text-violet-700 shadow-inner">
                        <Icon size={22} />
                      </div>
          ...
          <div className="relative min-h-[260px]">
            <div className="absolute inset-0 rounded-[2.2rem] bg-gradient-to-br from-orange-300/30 via-pink-300/16 to-violet-500/20 blur-2xl" />
            ...
            {panelQuotes[0] ? (
              <div className="absolute -bottom-4 right-4 max-w-sm rounded-[1.6rem] bg-gradient-to-br from-violet-700 via-fuchsia-600 to-orange-500 p-5 text-white shadow-[0_22px_55px_rgba(236,72,153,0.35)]">
                ...
                <div className="mt-2 text-[10px] font-black uppercase tracking-[0.16em] text-orange-100">— {panelQuotes[0].label}</div>
              </div>
            ) : null}
          </div>
        </div>
  ...
  return (
    <div className="relative overflow-hidden rounded-[2rem] bg-gradient-to-br from-[#1a0b2e] via-[#2a0f4f] to-[#080315] p-6 text-white shadow-[0_24px_70px_rgba(76,29,149,0.22)] ring-1 ring-white/10">
      ...
      <div className="relative grid gap-5 xl:grid-cols-[1fr_0.9fr] xl:items-center">
        <div>
          <div className="inline-flex items-center gap-2 rounded-full border border-orange-300/20 bg-white/8 px-4 py-1.5 text-[10px] font-black uppercase tracking-[0.2em] text-orange-200">
            <Sparkles size={13} />
            Premium Workspace
          </div>
...
export function VyronPremiumQuoteCard({ label, quote, className = "" }: { label: string; quote: string; className?: string }) {
  return (
    <div className={`relative overflow-hidden rounded-[1.8rem] bg-gradient-to-br from-violet-700 via-fuchsia-600 to-orange-500 p-6 text-white shadow-[0_22px_55px_rgba(236,72,153,0.30)] ${className}`}>
      <div className="pointer-events-none absolute -right-6 -top-6 text-[100px] font-black leading-none text-white/10">&ldquo;</div>
      <div className="relative text-[10px] font-black uppercase tracking-[0.16em] text-orange-100">{label}</div>
...
export function VyronPremiumSpotlightQuote({ quote, attribution }: { quote: string; attribution: string }) {
  return (
    <div className="relative overflow-hidden rounded-[2rem] bg-gradient-to-br from-orange-500 via-rose-500 to-violet-700 p-8 text-white shadow-[0_24px_65px_rgba(249,115,22,0.35)] md:p-10">
      ...
      <p className="relative mt-4 text-sm font-black uppercase tracking-[0.16em] text-orange-100">— {attribution}</p>
    </div>
  );
}
    ...
    >
      ...
      <div className="relative flex items-start justify-between gap-4">
        <div className="min-w-0 flex-1">
          <div className={`text-xs font-black uppercase tracking-[0.2em] ${isDark ? "text-orange-300" : "text-violet-600"}`}>{eyebrow}</div>
  ...
  return (
    <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
      <div className="flex items-start gap-4">
        {icon ? <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-violet-100 via-white to-orange-100 text-violet-700 shadow-inner">{icon}</div> : null}
        <div>
...
export function VyronPremiumEmptyState({ title = "Getting Started", steps }: { title?: string; steps: string[] }) {
  return (
    <div className="rounded-[2rem] border border-dashed border-orange-200/80 bg-gradient-to-br from-orange-50/60 via-violet-50/40 to-white p-8 text-center md:p-10">
      ...
      <ol className="mx-auto mt-6 max-w-lg space-y-3 text-left text-sm font-semibold leading-6 text-slate-700">
        {steps.map((step, index) => (
          <li key={step} className="flex gap-3 rounded-xl border border-white/80 bg-white/70 px-3 py-2">
            <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-violet-700 to-orange-500 text-xs font-black text-white">{index + 1}</span>
