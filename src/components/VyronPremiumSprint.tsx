      ...
      <div className="relative overflow-hidden rounded-[2.4rem] border border-white/70 bg-gradient-to-br to-[var(--vyron-warning-bg)] via-rose-50 to-blue-50 p-0 shadow-[0_28px_90px_rgba(30,58,138,0.13)]">
        ...
        <div className="relative grid min-h-[350px] gap-8 p-7 md:p-9 xl:grid-cols-[1fr_0.95fr] xl:items-center">
          <div className="max-w-3xl">
            <div className="inline-flex items-center gap-2 rounded-full border border-blue-200/70 bg-white/70 px-4 py-2 text-[10px] font-black uppercase tracking-[0.22em] text-blue-700 shadow-sm backdrop-blur-xl">
            ...
            <p className="mt-4 max-w-2xl text-sm font-black uppercase leading-7 tracking-[0.13em] text-blue-700">
              {subtitle}
            </p>
            ...
            {outcomes.length > 0 ? (
              <div className="mt-8 grid max-w-4xl gap-4 sm:grid-cols-2 xl:grid-cols-4">
                {outcomes.slice(0, 4).map((outcome, index) => {
                  ...
                  return (
                    <div key={outcome} className="group rounded-[1.5rem] border border-white/80 bg-white/72 p-4 shadow-[0_18px_45px_rgba(30,58,138,0.08)] backdrop-blur-xl">
                      <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br from-blue-100 via-white to-[var(--vyron-warning-bg)] text-blue-700 shadow-inner">
                        <Icon size={22} />
                      </div>
          ...
          <div className="relative min-h-[260px]">
            <div className="absolute inset-0 rounded-[2.2rem] bg-gradient-to-br to-[var(--vyron-warning-bg)] via-pink-300/16 to-blue-500/20 blur-2xl" />
            <div className="relative overflow-hidden rounded-[2.2rem] border border-white/60 bg-white/45 p-5 shadow-[0_24px_70px_rgba(30,58,138,0.16)] backdrop-blur-xl">
            ...
            {panelQuotes[0] ? (
              <div className="absolute -bottom-4 right-4 max-w-sm rounded-[1.6rem] bg-gradient-to-br from-blue-700 via-blue-600 to-[var(--vyron-warning-bg)] p-5 text-white shadow-[0_22px_55px_rgba(236,72,153,0.35)]">
...
export function VyronPremiumQuoteCard({ label, quote, className = "" }: { label: string; quote: string; className?: string }) {
  return (
    <div className={`relative overflow-hidden rounded-[1.8rem] bg-gradient-to-br from-blue-700 via-blue-600 to-[var(--vyron-warning-bg)] p-6 text-white shadow-[0_22px_55px_rgba(236,72,153,0.30)] ${className}`}>
...
export function VyronPremiumSpotlightQuote({ quote, attribution }: { quote: string; attribution: string }) {
  return (
    <div className="relative overflow-hidden rounded-[2rem] bg-gradient-to-br to-[var(--vyron-warning-bg)] via-rose-500 to-blue-700 p-8 text-white shadow-[0_24px_65px_rgba(249,115,22,0.35)] md:p-10">
    ...
    >
      ...
      <div className="relative flex items-start justify-between gap-4">
        <div className="min-w-0 flex-1">
          <div className={`text-xs font-black uppercase tracking-[0.2em] ${isDark ? "text-[var(--vyron-warning-fg)]" : "text-blue-600"}`}>{eyebrow}</div>
          ...
          <div className="mt-5 grid gap-3">
            {items.map((item) => (
              <div key={item.label} className="rounded-2xl border border-white/90 bg-white/85 p-4 shadow-sm">
                <div className="text-xs font-black uppercase tracking-[0.1em] text-blue-700">{item.label}</div>
...
export function VyronPremiumInsightCard({ category, message, href }: { category: string; message: string; href?: string }) {
  ...
  return (
    <li className="rounded-2xl border border-[var(--vyron-warning-border)] bg-gradient-to-r from-white to-[var(--vyron-warning-bg)] px-5 py-4 text-sm font-semibold leading-6 text-slate-800 shadow-sm">
      <span className="mr-2 inline-block rounded-lg bg-gradient-to-r to-[var(--vyron-warning-bg)] to-blue-100 px-2.5 py-0.5 text-[10px] font-black uppercase tracking-[0.08em] text-[var(--vyron-warning-fg)]">
        {category}
      </span>
      {body}
    </li>
  ...
  return (
    <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
      <div className="flex items-start gap-4">
        {icon ? <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-blue-100 via-white to-[var(--vyron-warning-bg)] text-blue-700 shadow-inner">{icon}</div> : null}
        <div>
          {eyebrow ? <div className="text-[10px] font-black uppercase tracking-[0.18em] text-blue-600">{eyebrow}</div> : null}
...
export function VyronPremiumMetricCard({ label, value, href, icon, note, tone = "default" }: { label: string; value: ReactNode; href: string; icon?: ReactNode; note?: string; tone?: "default" | "warning" | "danger" }) {
  const toneClass =
    tone === "danger"
      ...
      : tone === "warning"
        ? "border-[var(--vyron-warning-border)] bg-gradient-to-br from-blue-50/90 to-white"
        : "border-blue-100/80 bg-gradient-to-br from-white via-blue-50/30 to-[var(--vyron-warning-bg)]";
  return (
    <Link href={href} className={`rounded-[2rem] border p-6 shadow-[0_18px_60px_rgba(30,58,138,0.08)] transition hover:-translate-y-0.5 hover:border-blue-300 hover:shadow-[0_26px_70px_rgba(30,58,138,0.14)] ${toneClass}`}>
      {icon ? <div className="text-blue-600">{icon}</div> : null}
      <div className={`text-[10px] font-black uppercase tracking-[0.12em] text-blue-600 ${icon ? "mt-3" : ""}`}>{label}</div>
...
export function VyronPremiumInsightsPanel({ title, icon, children, empty }: { title: string; icon?: ReactNode; children: ReactNode; empty?: ReactNode }) {
  return (
    <div className="rounded-[2rem] border border-[var(--vyron-warning-border)] bg-gradient-to-br to-[var(--vyron-warning-bg)] via-blue-50/40 to-white p-6 md:p-8">
...
export function VyronPremiumEmptyState({ title = "Getting Started", steps }: { title?: string; steps: string[] }) {
  return (
    <div className="rounded-[2rem] border border-dashed border-[var(--vyron-warning-border)] bg-gradient-to-br to-[var(--vyron-warning-bg)] via-blue-50/40 to-white p-8 text-center md:p-10">
      ...
      <ol className="mx-auto mt-6 max-w-lg space-y-3 text-left text-sm font-semibold leading-6 text-slate-700">
        {steps.map((step, index) => (
          <li key={step} className="flex gap-3 rounded-xl border border-white/80 bg-white/70 px-3 py-2">
            <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-blue-700 to-[var(--vyron-warning-bg)] text-xs font-black text-white">{index + 1}</span>
