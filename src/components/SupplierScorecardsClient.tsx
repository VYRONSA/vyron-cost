  ...
  return (
    ...
    >
      <section className="grid gap-6">
        ...
        {filtered.map((row) => (
          <Link key={row.id} href={row.href} className="rounded-[2rem] bg-white p-6 shadow-[0_10px_40px_rgba(15,23,42,0.06)] transition hover:-translate-y-1 hover:bg-[#3B82F6]/10">
            ...
            <div className="mt-6 grid gap-3">
              ...
              <div className="flex justify-between rounded-2xl border border-[#3B82F6]/20 bg-[#3B82F6]/10 px-4 py-3 text-sm">
                <span className="font-bold text-[#4D7C0F]">Negotiation</span>
                <span className="font-black text-[#1D4ED8]">{formatSupplierSpend(row.negotiation_opportunity)}</span>
              </div>
