...
export default async function IngredientDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!ingredient) return <VyronCostAiShell title="Ingredient Not Found"><div className="rounded-[2rem] bg-white p-8">Ingredient not found.</div></VyronCostAiShell>;
  const movement = calculateMovementPercent(Number(ingredient.previous_cost || 0), Number(ingredient.purchase_cost || 0));
  return (
    <VyronCostAiShell title={ingredient.ingredient_name} subtitle="Ingredient detail, true cost and movement.">
      <section className="grid gap-5 md:grid-cols-4">
        ...
        <div className="rounded-[2rem] bg-white p-6"><div className="text-xs font-black uppercase text-slate-400">Movement</div><div className={`mt-3 text-3xl font-black ${movement > 5 ? "text-red-600" : "text-emerald-600"}`}>{movement.toFixed(1)}%</div></div>
      </section>
    </VyronCostAiShell>
  );
}
