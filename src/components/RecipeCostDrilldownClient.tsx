  ...
  return (
    <section className="grid gap-6">
      <div className="rounded-[2rem] bg-[#07110d] p-6 text-white shadow-[0_18px_55px_rgba(6,20,14,0.24)]">
        <Boxes size={34} className="text-emerald-300" />
        ...
        {rows.map(({ product, ingredient, packaging, labour, overhead, total }) => (
          <div key={product.id} className="rounded-[2rem] bg-white p-6 shadow-[0_10px_40px_rgba(15,23,42,0.06)]">
            <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
              <div>
                <h3 className="text-2xl font-black text-[#07110d]">{product.product_name}</h3>
                <p className="mt-1 text-sm font-bold text-slate-500">{product.category} · Total cost {money(total)}</p>
              </div>
              <Link href={product.product_id ? `/products/${product.product_id}` : "/products"} className="inline-flex items-center gap-2 rounded-2xl bg-emerald-50 px-5 py-3 text-sm font-black text-emerald-800">
            ...
            <div className="mt-5 grid gap-3 md:grid-cols-4">
              {[
                ["Ingredients", ingredient, "bg-emerald-50 text-emerald-800"],
