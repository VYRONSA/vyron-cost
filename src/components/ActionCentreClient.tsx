      ...
      <div className="space-y-3">
        {items.map((item) => (
          <Link
            key={item.id}
            href={item.href}
            className="flex items-center justify-between gap-4 rounded-2xl border border-slate-100 bg-slate-50 px-5 py-4 transition hover:border-emerald-200"
          >
            <div>
              <div className="font-black text-[#07110d]">{item.title}</div>
              <div className="mt-1 text-sm text-slate-500">{item.detail}</div>
            </div>
            <ArrowUpRight size={18} className="shrink-0 text-emerald-700" />
          </Link>
        ))}
      </div>
    </section>
