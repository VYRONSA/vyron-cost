  ...
  return (
    ...
    >
      <section className="grid gap-6">
        ...
        {filtered.map((row) => (
          <Link key={row.id} href={row.href} className="rounded-[2rem] bg-white p-6 shadow-[0_10px_40px_rgba(11,32,43,0.06)] transition hover:-translate-y-1 hover:bg-[#2C5A6B]/10">
            ...
            <div className="mt-6 grid gap-3">
              ...
              <div className="flex justify-between rounded-2xl border border-[#2C5A6B]/20 bg-[#2C5A6B]/10 px-4 py-3 text-sm">
                <span className="font-bold text-[#2F7C40]">Negotiation</span>
                <span className="font-black text-[#163A48]">{formatSupplierSpend(row.negotiation_opportunity)}</span>
              </div>
