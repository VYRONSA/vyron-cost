...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!companyId) return NextResponse.json({ ok: true, boms: [] }, { headers: { "Cache-Control": "no-store" } });
    const boms = await listBomsForProduction(supabase, companyId);
    return NextResponse.json({ ok: true, boms }, { headers: { "Cache-Control": "no-store" } });
  } catch (error) {
