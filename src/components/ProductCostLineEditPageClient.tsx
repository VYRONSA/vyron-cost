    ...
    <section className="grid gap-6 xl:grid-cols-[1.1fr_0.75fr]">
      <div className="rounded-[2rem] border border-white bg-white p-7 shadow-[0_10px_40px_rgba(15,23,42,0.06)]">
        <div className="mb-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <h2 className="text-3xl font-black text-[#07110d]">Edit Product Cost Line</h2>
        ...
        <div className="grid gap-5">
          <label className="text-sm font-black text-slate-600">
            Line Type
            <select className="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-4 text-base font-bold outline-none focus:border-emerald-400" value={form.line_type} onChange={(event) => updateForm("line_type", event.target.value)}>
          ...
          <label className="text-sm font-black text-slate-600">
            Line Name
            <input className="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-4 text-base font-bold outline-none focus:border-emerald-400" value={form.line_name} onChange={(event) => updateForm("line_name", event.target.value)} />
          </label>

          <div className="grid gap-5 md:grid-cols-2">
            <label className="text-sm font-black text-slate-600">
              Quantity
              <input type="number" className="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-4 text-base font-bold outline-none focus:border-emerald-400" value={form.quantity} onChange={(event) => updateForm("quantity", event.target.value)} />
            </label>

            <label className="text-sm font-black text-slate-600">
              Unit
              <input className="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-4 text-base font-bold outline-none focus:border-emerald-400" value={form.unit} onChange={(event) => updateForm("unit", event.target.value)} />
            </label>
          </div>
          ...
          <div className="grid gap-5 md:grid-cols-2">
            <label className="text-sm font-black text-slate-600">
              Unit Cost
              <input type="number" className="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-4 text-base font-bold outline-none focus:border-emerald-400" value={form.unit_cost} onChange={(event) => updateForm("unit_cost", event.target.value)} />
            </label>
      ...
      <aside className="rounded-[2rem] bg-[#07110d] p-7 text-white shadow-[0_18px_55px_rgba(6,20,14,0.24)]">
        ...
        <div className="mt-6 rounded-3xl border border-emerald-400/15 bg-white/5 p-5">
          <div className="text-sm font-black text-emerald-300">Product</div>
