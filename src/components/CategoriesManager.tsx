    ...
    <section className="grid gap-6 xl:grid-cols-[0.8fr_1.5fr]">
      <div className="rounded-[2rem] border border-white bg-white p-6 shadow-[0_10px_40px_rgba(11,32,43,0.06)]">
        <div className="mb-5 flex items-center gap-3">
          <div className="rounded-2xl border border-[#2C5A6B]/20 bg-[#2C5A6B]/10 p-3 text-[#55B968]">{editingId ? <Edit3 size={20} /> : <Plus size={20} />}</div>
        ...
        <div className="grid gap-4">
          ...
          {(message || savingFinancial) && <div className="rounded-2xl border border-[#2C5A6B]/20 bg-[#2C5A6B]/10 px-4 py-3 text-sm font-bold text-[#163A48]">{savingFinancial ? "Saving category financial mapping..." : message}</div>}
        </div>
      </div>
      <div className="rounded-[2rem] border border-white bg-white p-6 shadow-[0_10px_40px_rgba(11,32,43,0.06)]">
        ...
        <div className="overflow-x-auto rounded-3xl border border-slate-100"><div className="min-w-[900px]"><div className="grid grid-cols-6 bg-[#061722] px-5 py-4 text-xs font-black uppercase tracking-[0.16em] text-[#2C5A6B]"><div>Name</div><div>Type</div><div>Description</div><div>Status</div><div>Edit</div><div>Delete</div></div>{filtered.map((category) => (<div key={category.id} className="grid grid-cols-6 items-center border-t border-slate-100 px-5 py-5 text-sm"><div className="font-black text-[#F8FAFC]">{category.category_name}</div><div className="font-bold text-slate-600">{category.category_type}</div><div className="text-slate-500">{category.description || "No description"}</div><div><StatusPill tone={statusTone(category.status)}>{category.status}</StatusPill></div><div><button type="button" onClick={() => startEdit(category)} className="inline-flex items-center gap-2 rounded-full border border-[#2C5A6B]/25 bg-[#2C5A6B]/10 px-3 py-2 text-xs font-black text-[#163A48]"><Edit3 size={14} />Edit</button></div><div><button type="button" onClick={() => deleteCategory(category.id)} className="inline-flex items-center gap-2 rounded-full bg-red-50 px-3 py-2 text-xs font-black text-red-700"><Trash2 size={14} />Delete</button></div></div>))}</div></div>
      </div>
    </section>
  );
}
