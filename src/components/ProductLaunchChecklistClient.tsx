      ...
      {checks.map((check) => {
        ...
        return (
          <button key={check} onClick={() => setDone((current) => ({ ...current, [check]: !checked }))} className="flex items-center gap-4 rounded-[2rem] bg-white p-5 text-left shadow-[0_10px_40px_rgba(15,23,42,0.06)]">
            <span className={`flex h-12 w-12 items-center justify-center rounded-2xl ${checked ? "bg-emerald-500 text-white" : "bg-slate-100 text-slate-400"}`}>
              <CheckCircle2 size={24} />
            </span>
            <span className="text-lg font-black text-[#07110d]">{check}</span>
          </button>
        );
      })}
    </section>
