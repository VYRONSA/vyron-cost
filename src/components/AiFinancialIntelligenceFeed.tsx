...
const signalStyles = {
  ...
  recover: "border-[#BCCDD5] bg-[#EEF8F0]",
        ...
        <div className="border-b border-[#E2E8F0] px-8 py-6">
          <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex items-center gap-4">
              <div className="flex h-12 w-12 items-center justify-center rounded-2xl border border-[#E2E8F0] bg-[#F8FAFC] text-[#0B202B]">
              ...
              <div>
                ...
                <h2 className="mt-1 text-2xl font-black text-[#0B202B]">Intelligence Timeline</h2>
              </div>
            </div>
            <div className="flex gap-4">
              <div className="rounded-2xl border border-[#FECACA] bg-[#FEF2F2] px-5 py-4">
                ...
                <div className="mt-1 text-lg font-black text-[#0B202B]">{formatMoney(totalLoss)}</div>
              </div>
              <div className="rounded-2xl border border-[#BCCDD5] bg-[#EEF8F0] px-5 py-4">
                <div className="text-[10px] font-black uppercase tracking-[0.16em] text-[#1F4757]">Recoverable</div>
                <div className="mt-1 text-lg font-black text-[#1F4757]">{formatMoney(totalRecoverable)}</div>
              </div>
            </div>
          </div>
        </div>
          ...
          {items.map((item) => {
            ...
            return (
              <Link
                key={item.id}
                href={item.href}
                className={`group block rounded-2xl border p-5 transition hover:shadow-[0_12px_32px_rgba(11,32,43,0.08)] ${signalStyles[signal]}`}
              >
                <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                  <div className="flex min-w-0 flex-1 items-start gap-4">
                    ...
                    <div>
                      ...
                      <div className="mt-1 text-lg font-black text-[#0B202B]">{item.headline}</div>
                  ...
                  <div className="flex items-center gap-6">
                    ...
                    {item.recoverableAmount > 0 && (
                      <div className="text-sm font-black text-[#1F4757]">{formatMoney(item.recoverableAmount)}</div>
                    )}
                    <span className="inline-flex items-center gap-1 text-xs font-black uppercase tracking-[0.12em] text-[#64748B] group-hover:text-[#0B202B]">
                      {item.action}
