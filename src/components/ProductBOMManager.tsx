...
function inputClass() {
  return "mt-2 w-full rounded-2xl border border-slate-200 px-4 py-3 font-bold outline-none transition focus:border-blue-400 focus:ring-4 focus:ring-blue-100";
}
      ...
      <div className="grid gap-6 xl:grid-cols-[0.8fr_1.5fr]">
        ...
        <div className={`rounded-[2rem] border border-white bg-white p-6 shadow-[0_18px_55px_rgba(22,58,72,0.08)] ${readOnly ? "xl:col-span-2" : ""}`}>
          ...
          {productLines.length === 0 ? (
            <div className="mb-5 rounded-2xl border border-dashed border-blue-200 bg-blue-50/60 px-5 py-6">
              <div className="text-sm font-black text-blue-800">No cost lines captured yet</div>
              ...
              {productLines.map((line) => (
                <div key={line.id} className={`grid ${readOnly ? "grid-cols-7" : "grid-cols-9"} items-center border-t border-slate-100 px-5 py-5 text-sm`}>
                  ...
                  {!readOnly ? (
                    <>
                      <div>
                        <Link href={`/products/${product.id}/cost-lines/${line.id}/edit`} className="inline-flex items-center gap-2 rounded-full bg-blue-50 px-3 py-2 text-xs font-black text-blue-700">
                          <Edit3 size={14} />
                          Open Edit Page
                        </Link>
                      </div>
