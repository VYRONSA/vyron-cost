...
export default function InventoryStockDetailClient({ stockItemId }: { stockItemId: string }) {
  ...
  const load = useCallback(async () => {
    const [stockRes, ledgerRes] = await Promise.all([
      fetch(`/api/inventory/stock/${stockItemId}`).then((r) => r.json()),
      fetch(`/api/inventory/ledger?stockItemId=${stockItemId}`).then((r) => r.json()),
    ]);
  ...
  if (!item) return <p className="text-sm font-bold text-slate-500">Loading stock item…</p>;
