...
import { importSuppliersFromXero } from "@/lib/vyron-xero-import-suppliers";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const result = await importSuppliersFromXero(supabase, workspaceId, companyId, actor);

    await appendXeroAuditEvent(
      workspaceId,
      {
        event: "suppliers_imported",
        actor,
        companyId,
        detail: `Imported ${result.imported + result.updated} suppliers from Xero (${result.imported} new, ${result.updated} updated, ${result.skipped} skipped).`,
        metadata: result,
      },
      companyId
    );
    ...
    return workspaceAccessErrorResponse(error, "Xero supplier import failed.");
  }
}
