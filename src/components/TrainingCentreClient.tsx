...
const sections = [
  ...
  ["6. Use AI", "Ask VOLORA which products are below GP, which suppliers increased prices and what to do today."],
];

function downloadManual() {
  const text = [
    "VOLORA — Handcrafted Foods Demo Manual",
    "",
            ...
            <section className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
              {sections.map(([title, detail]) => (
                <div key={title} className="rounded-[2rem] bg-white p-6 shadow-[0_10px_40px_rgba(11,32,43,0.06)]">
                  <PlayCircle size={28} className="text-[#163A48]" />
