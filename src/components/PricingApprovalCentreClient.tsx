    ...
    <section className="grid gap-6">
      <section className="grid gap-5 md:grid-cols-3">
        ...
        <div className="rounded-[2rem] bg-emerald-50 p-6 shadow-[0_10px_40px_rgba(15,23,42,0.06)]">
          <div className="text-xs font-black uppercase tracking-[0.16em] text-emerald-700">Approved Monthly Impact</div>
          <div className="mt-3 text-4xl font-black text-emerald-700">{money(approvedMonthly)}</div>
        </div>
        <div className="rounded-[2rem] bg-[#07110d] p-6 text-white shadow-[0_18px_55px_rgba(6,20,14,0.24)]">
          <div className="text-xs font-black uppercase tracking-[0.16em] text-emerald-300">Annualised</div>
        ...
        <div className="min-w-[1180px]">
          ...
          {rows.map((row) => (
            <div key={row.id} className="grid grid-cols-10 items-center border-t border-slate-100 px-5 py-5 text-sm">
              ...
              <div className="font-black text-emerald-700">{money(row.monthlyImpact)}</div>
              ...
              <div className="flex gap-2">
                <button onClick={() => setDecisions((current) => ({ ...current, [row.id]: "Approved" }))} className="rounded-full bg-emerald-50 p-2 text-emerald-700"><CheckCircle2 size={16} /></button>
