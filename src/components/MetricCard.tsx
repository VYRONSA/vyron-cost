  ...
  return (
    <div className={`rounded-[2rem] p-5 shadow-[0_10px_40px_rgba(15,23,42,0.06)] ${dark ? "bg-[#0b1210] text-white" : "border border-white bg-white text-slate-950"}`}>
      <div className="flex items-start justify-between">
        <div className={dark ? "rounded-2xl bg-white/10 p-3" : "rounded-2xl bg-emerald-50 p-3"}>
          <Icon className={dark ? "text-emerald-300" : "text-emerald-600"} size={22} />
        </div>
      ...
      <div className={dark ? "mt-2 text-sm font-bold text-emerald-300" : "mt-2 text-sm font-bold text-emerald-600"}>{note}</div>
    </div>
  );
}
