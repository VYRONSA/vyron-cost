    ...
    <div className="space-y-6">
        ...
        {hubLinks.map((link) => (
          <Link
            ...
            className={`rounded-full px-4 py-2 text-xs font-black ${
              link.active ? "bg-violet-700 text-white" : "bg-slate-100 text-slate-700 hover:bg-slate-200"
            }`}
          >
            {link.label}
          </Link>
      ...
      <section className="rounded-[2rem] border border-violet-100 bg-white p-5">
        ...
        <button
          type="button"
          onClick={() => void load()}
          className="mt-3 rounded-xl bg-violet-700 px-4 py-2 text-xs font-black text-white"
        >
          Apply filters
        </button>
      </section>
