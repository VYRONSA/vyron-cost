    ...
    <section className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
      <div className="space-y-6">
        <div className="rounded-[2rem] border border-white bg-white p-6 shadow-sm">
          ...
          <h2 className="mt-2 text-3xl font-black text-[#07110d]">{detail.opportunity}</h2>
          <p className="mt-4 text-sm leading-7 text-slate-600">{detail.whyDetected}</p>
        </div>

        <div className="rounded-[2rem] border border-emerald-200 bg-emerald-50 p-6">
          <div className="text-xs font-black uppercase tracking-[0.16em] text-emerald-800">Formula used</div>
          <div className="mt-2 text-lg font-black text-[#07110d]">{detail.formulaName}</div>
          <div className="mt-4 rounded-xl bg-white p-4 font-mono text-sm font-bold text-slate-800">
            {detail.formulaExpression}
          </div>
          <div className="mt-3 text-sm font-bold text-emerald-900">{detail.formulaWorkedExample}</div>
          <div className="mt-4 grid gap-3 md:grid-cols-2">
            <div className="rounded-xl bg-white p-4">
              <div className="text-xs font-black uppercase text-slate-400">Monthly value</div>
              <div className="mt-1 text-2xl font-black text-emerald-700">{formatMoney(detail.monthly_saving)}</div>
            </div>
            <div className="rounded-xl bg-white p-4">
              <div className="text-xs font-black uppercase text-slate-400">Annual value</div>
              <div className="mt-1 text-2xl font-black text-emerald-700">{formatMoney(detail.annual_saving)}</div>
        ...
        <div className="rounded-[2rem] border border-white bg-white p-6 shadow-sm">
          <h3 className="text-lg font-black text-[#07110d]">Products affected</h3>
            ...
            <div className="mt-4 space-y-2">
              {detail.productsAffected.map((product) => (
                <Link
                  key={product.id}
                  href={product.href}
                  className="flex items-center justify-between rounded-xl bg-slate-50 px-4 py-3 transition hover:bg-emerald-50"
                >
                  <span className="font-black text-[#07110d]">{product.name}</span>
        ...
        <div className="rounded-[2rem] border border-white bg-white p-6 shadow-sm">
          <h3 className="text-lg font-black text-[#07110d]">Suppliers affected</h3>
            ...
            <div className="mt-4 space-y-2">
              {detail.suppliersAffected.map((supplier) => (
                <Link
                  ...
                  className="flex items-center justify-between rounded-xl bg-slate-50 px-4 py-3 transition hover:bg-emerald-50"
                >
                  <span className="font-black text-[#07110d]">{supplier.name}</span>
      ...
      <aside className="space-y-5">
        <div className="rounded-[2rem] bg-[#08111A] p-6 text-white">
          ...
          <div className="mt-3 text-4xl font-black text-emerald-300">{formatMoney(detail.annual_saving)}</div>
        ...
        <div className="rounded-[2rem] border border-white bg-white p-6 shadow-sm">
          ...
          <div className="mt-4 grid gap-3">
            <button
              ...
              className="inline-flex items-center justify-center gap-2 rounded-xl bg-emerald-500 px-4 py-3 text-sm font-black text-[#07110d]"
            >
            ...
            <button
              ...
              className="inline-flex items-center justify-center gap-2 rounded-xl bg-amber-100 px-4 py-3 text-sm font-black text-amber-900"
            >
              <Search size={16} />
          ...
          {message ? (
            <div className="mt-4 rounded-xl bg-emerald-50 px-4 py-3 text-sm font-black text-emerald-700">{message}</div>
          ) : null}
        </div>
      </aside>
    </section>
