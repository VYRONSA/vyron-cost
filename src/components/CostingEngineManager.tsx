      ...
      <div className="rounded-[2rem] border border-white bg-white p-6 shadow-[0_10px_40px_rgba(15,23,42,0.06)]">
        ...
        <div className="grid gap-4">
          ...
          <div className="rounded-3xl bg-[#07110d] p-5 text-white">
            <div className="text-xs font-black uppercase tracking-[0.25em] text-[#A855F7]">Line Cost Preview</div>
          ...
          <div className="min-w-[980px]">
            ...
            {filteredItems.map((item) => (
              <div key={item.id} className="grid grid-cols-7 items-center border-t border-slate-100 px-5 py-5 text-sm">
                ...
                <div className="font-black text-[#7E22CE]">{formatMoney(Number(item.line_cost))}</div>
                <div>
                  <Link href={`/cost-calculator/${item.id}/edit`} className="inline-flex items-center gap-2 rounded-full border border-[#A855F7]/25 bg-[#A855F7]/10 px-3 py-2 text-xs font-black text-[#7E22CE]">
                    <Edit3 size={14} />
                    Open Edit Page
                  </Link>
                </div>
