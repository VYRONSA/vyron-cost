    ...
    <section className="grid gap-6 xl:grid-cols-[1.1fr_0.75fr]">
      <div className="rounded-[2rem] border border-white bg-white p-7 shadow-[0_10px_40px_rgba(15,23,42,0.06)]">
        <div className="mb-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <h2 className="text-3xl font-black text-[#07110d]">Edit Recipe</h2>
        ...
        <div className="grid gap-5">
          <label className="text-sm font-black text-slate-600">
            Recipe Name
            <input className="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-4 text-base font-bold outline-none focus:border-emerald-400" value={form.recipe_name} onChange={(event) => updateForm("recipe_name", event.target.value)} />
          </label>

          <div className="grid gap-5 md:grid-cols-2">
            <label className="text-sm font-black text-slate-600">
              Recipe Type
              <select className="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-4 text-base font-bold outline-none focus:border-emerald-400" value={form.recipe_type} onChange={(event) => updateForm("recipe_type", event.target.value)}>
            ...
            <label className="text-sm font-black text-slate-600">
              Status
              <select className="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-4 text-base font-bold outline-none focus:border-emerald-400" value={form.status} onChange={(event) => updateForm("status", event.target.value)}>
          ...
          <div className="grid gap-5 md:grid-cols-4">
            <label className="text-sm font-black text-slate-600">
              Yield Qty
              <input type="number" className="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-4 text-base font-bold outline-none focus:border-emerald-400" value={form.yield_qty} onChange={(event) => updateForm("yield_qty", event.target.value)} />
            </label>
          ...
          <label className="text-sm font-black text-slate-600">
            Version Note
            <textarea className="mt-2 min-h-36 w-full rounded-2xl border border-slate-200 px-4 py-4 text-base font-medium outline-none focus:border-emerald-400" value={form.version_note} onChange={(event) => updateForm("version_note", event.target.value)} />
          </label>

          <div className="flex flex-wrap gap-3">
            <button type="button" onClick={saveRecipe} className="inline-flex items-center gap-2 rounded-2xl bg-emerald-500 px-6 py-4 text-sm font-black text-[#07110d] transition hover:bg-emerald-400">
              <Save size={18} />
              Save Recipe
            </button>

            <button type="button" onClick={deleteRecipe} className="inline-flex items-center gap-2 rounded-2xl bg-red-50 px-6 py-4 text-sm font-black text-red-700 transition hover:bg-red-100">
              <Trash2 size={18} />
              Delete Recipe
            </button>
          </div>
