...
const CATEGORY_COLORS: Record<string, string> = {
  "Supplier Change": "bg-blue-100 text-blue-900",
  Recovery: "bg-[#3B82F6]/12 text-[#4D7C0F]",
    ...
    >
      <section className="relative border-l-2 border-slate-200 pl-8">
        ...
        <article key={e.id} className="relative mb-8 pb-2">
          <span className="absolute -left-[2.05rem] top-1 h-4 w-4 rounded-full border-4 border-white bg-blue-600 shadow" />
          ...
          {e.href ? (
            <Link href={e.href} className="mt-2 inline-block text-xs font-black text-blue-700">
              Open record →
            </Link>
          ) : null}
        </article>
