      ...
      <section className="mt-5 rounded-[2rem] bg-white p-6 shadow-[0_18px_50px_rgba(29,78,216,0.08)]">
        ...
        <div className="space-y-3">
          {categories.map((category) => {
            ...
            return (
              <Link key={category.name} href={category.href} className="block rounded-2xl border border-slate-100 p-4 transition hover:bg-blue-50">
                <div className="flex items-center justify-between gap-4">
                  <div className="font-black text-slate-900">{category.name}</div>
                  <div className="font-black text-blue-700">{formatMoney(category.value)}</div>
                </div>
