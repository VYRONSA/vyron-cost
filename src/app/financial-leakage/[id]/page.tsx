...
export default async function FinancialLeakageDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!detail) {
    return (
      <VyronCostAiShell hidePageHeader title="Recovery Detail Not Found" subtitle="RECOVERY INTELLIGENCE · DRILLDOWN">
        <div className="rounded-[2rem] border border-blue-100 bg-white p-8 font-bold text-slate-600 shadow-[0_18px_60px_rgba(30,58,138,0.08)]">
  ...
  return (
    <VyronCostAiShell hidePageHeader title={title} subtitle={subtitle}>
      <section className="relative overflow-hidden rounded-[2.4rem] border border-blue-100 bg-white p-7 shadow-[0_18px_60px_rgba(30,58,138,0.08)]">
      ...
      <section className="grid min-w-0 gap-6 lg:grid-cols-1 xl:grid-cols-[1fr_0.85fr] xl:items-start">
        <div className="min-w-0 overflow-hidden rounded-[2.4rem] border border-blue-100 bg-white p-6 shadow-[0_18px_60px_rgba(30,58,138,0.08)]">
          <div className="text-xs font-black uppercase tracking-[0.18em] text-blue-700">Formula & inputs</div>
          <h2 className="mt-1 text-2xl font-black text-slate-950">How this recovery is calculated</h2>

          <div className="mt-5 rounded-3xl bg-gradient-to-br from-blue-800 to-indigo-800 p-6 text-white shadow-lg shadow-blue-500/20">
            <div className="text-xs font-black uppercase tracking-[0.16em] text-blue-200">Formula</div>
            ...
            <div className="mt-8">
              ...
              <div className="mt-4 overflow-x-auto rounded-2xl border border-blue-100">
                <div className="min-w-[720px]">
                  <div className="grid grid-cols-5 bg-blue-800 px-4 py-3 text-xs font-black uppercase tracking-[0.14em] text-blue-100">
                    <div>Supplier</div>
                    <div>Invoice #</div>
                    <div>Amount</div>
                    <div>Matches</div>
                  ...
                  {detail.matchingInvoices.map((row) => (
                    ...
                    >
                      ...
                      <div className="font-black text-blue-700">{row.invoice_number}</div>
        ...
        <div className="min-w-0 rounded-[2.4rem] border border-blue-100 bg-white p-6 shadow-[0_18px_60px_rgba(30,58,138,0.08)]">
          ...
          <p className="mt-4 rounded-2xl border border-blue-100 bg-blue-50 p-4 text-sm font-black leading-7 text-blue-900">
          ...
          <Link
            href="/financial-leakage"
            className="mt-6 inline-flex items-center gap-2 rounded-2xl vyron-grad-surface px-5 py-3 text-sm font-semibold text-white shadow-lg shadow-blue-500/20"
          >
            ← Back
          </Link>
        </div>
