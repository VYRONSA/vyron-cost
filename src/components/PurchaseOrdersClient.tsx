    ...
    <section className="grid gap-6">
      <div className="grid gap-5 md:grid-cols-4">
        <div className="rounded-[2rem] border border-white bg-white p-6 shadow-[0_10px_40px_rgba(15,23,42,0.06)]">
          ...
          <div className="mt-3 text-4xl font-black text-[#07110d]">{openCount}</div>
        </div>
        <div className="rounded-[2rem] border border-white bg-white p-6 shadow-[0_10px_40px_rgba(15,23,42,0.06)]">
        ...
        <Link
          ...
          className="flex items-center justify-center gap-2 rounded-[2rem] bg-[#07110d] p-6 text-sm font-black text-emerald-300 transition hover:bg-[#0d1a12]"
        >
          <Plus size={18} />
          Create New PO
        </Link>
      ...
      <div className="rounded-[2rem] border border-white bg-white p-6 shadow-[0_10px_40px_rgba(15,23,42,0.06)]">
        <div className="mb-5 flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          <div>
            <h2 className="text-2xl font-black text-[#07110d]">Purchase Order Register</h2>
          ...
          <div className="min-w-[1100px]">
            ...
            {filtered.map((po) => (
              <div key={po.id} className="grid grid-cols-8 items-center border-t border-slate-100 px-5 py-4 text-sm">
                <div>
                  <Link href={`/purchase-orders/${po.id}`} className="font-black text-[#07110d] hover:text-emerald-700">
                    {po.po_number}
                  </Link>
                </div>
                ...
                <div className="flex flex-wrap gap-2">
                  <Link href={`/purchase-orders/${po.id}`} className="inline-flex items-center gap-1 rounded-full bg-emerald-50 px-3 py-2 text-xs font-black text-emerald-700">
                    <Edit3 size={14} />
                    Open
                  </Link>
