  ...
  return (
    <section className="grid gap-8">
          ...
          {profile.priceHistory.latest ? (
            <div className="mt-4 grid gap-2 rounded-xl bg-slate-50 p-4 text-sm sm:grid-cols-4">
              ...
              <div>
                ...
                <div className={`font-black ${profile.priceHistory.latest.percentage >= 0 ? "text-red-600" : "text-emerald-600"}`}>
            ...
            profile.benchmarks.map((b) => (
              <div key={b.itemName} className="rounded-xl border border-slate-100 p-4">
                ...
                <div className="mt-2 flex flex-wrap gap-4 text-xs font-bold text-slate-600">
                  ...
                  <span className="text-emerald-700">Potential saving: {money(b.potentialSaving)}/mo est.</span>
                </div>
              </div>
            ))
          ) : (
        ...
        <div className="rounded-[2rem] bg-white p-6 shadow-sm">
          ...
          <ul className="mt-4 space-y-2">
            {profile.risk.factors.length ? (
              ...
              <li className="text-sm font-bold text-emerald-700">No elevated risk factors.</li>
            )}
          </ul>
        </div>
      </section>
      ...
      <section className="rounded-[2rem] bg-emerald-50 p-6">
        <h3 className="text-lg font-black text-emerald-950">Savings Opportunities</h3>
        <div className="mt-4 space-y-2">
          {profile.savingsOpportunities.map((o) => (
            <div key={o.id} className="flex flex-wrap items-center justify-between gap-2 rounded-xl bg-white p-4">
              <div>
                <div className="text-xs font-black uppercase text-emerald-700">{o.type}</div>
                <div className="font-black text-slate-900">{o.title}</div>
              </div>
              <div className="text-right">
                <div className="font-black text-emerald-700">{money(o.potentialAnnual)}/yr</div>
