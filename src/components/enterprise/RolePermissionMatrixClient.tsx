    ...
    >
      <section className="overflow-x-auto rounded-[2rem] bg-white shadow-sm">
            <table className="min-w-[1200px] w-full text-sm">
              <thead>
                <tr className="bg-slate-900 text-left text-[10px] font-black uppercase tracking-wider text-[#A3E635]">
                      ...
                      <tr key={`${role.roleKey}-${mod.moduleKey}`} className="border-t border-slate-100">
                        ...
                        {ACTIONS.map((action) => (
                          <td key={action} className="p-3 text-center">
                            {mod[action] ? (
                              <span className="inline-block h-3 w-3 rounded-full border border-[#A3E635]/25 bg-[#A3E635]/100" />
                            ) : (
