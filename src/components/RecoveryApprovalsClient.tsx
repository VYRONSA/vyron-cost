  ...
  return (
    <VyronPremiumPageShell
      config={{
        ...
        subtitle: "Premium VOLORA workflow for recovery approvals.",
      ...
      <section className="grid gap-6">
            <section className="grid gap-5 md:grid-cols-3">
              ...
              <div className="rounded-[2rem] bg-[#2C5A6B]/10 p-6 shadow-[0_10px_40px_rgba(11,32,43,0.06)]">
                <div className="text-xs font-black uppercase tracking-[0.16em] text-[#163A48]">Approved Monthly Recovery</div>
                <div className="mt-3 text-4xl font-black text-[#163A48]">{money(approvedMonthly)}</div>
              </div>
              <div className="rounded-[2rem] bg-[#061722] p-6 text-white shadow-[0_18px_55px_rgba(6,20,14,0.24)]">
                <div className="text-xs font-black uppercase tracking-[0.16em] text-[#2C5A6B]">Annualised Approved</div>
              ...
              <div className="min-w-[1100px]">
                ...
                {rows.map((row) => (
                  <div key={row.id} className="grid grid-cols-9 items-center border-t border-slate-100 px-5 py-5 text-sm">
                    <div className="font-black text-[#163A48]">{row.finding_type}</div>
                    <div className="col-span-2">
                      <div className="font-black text-[#F8FAFC]">{row.title}</div>
                      <div className="text-xs text-slate-500">{row.description}</div>
                    </div>
                    <div className="font-black text-[#163A48]">{money(row.recovery)}</div>
                    ...
                    <div className="col-span-2 flex gap-2">
                      <button onClick={() => setDecisions((current) => ({ ...current, [row.id]: "Approved" }))} className="inline-flex items-center gap-2 rounded-full border border-[#2C5A6B]/25 bg-[#2C5A6B]/10 px-3 py-2 text-xs font-black text-[#163A48]">
