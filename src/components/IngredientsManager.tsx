      ...
      <div className="rounded-[2rem] border border-white bg-white p-6 shadow-[0_10px_40px_rgba(15,23,42,0.06)]">
        ...
        <div className="grid gap-4">
          ...
          <div className="rounded-3xl bg-[#07110d] p-5 text-white">
            <div className="text-xs font-black uppercase tracking-[0.25em] text-[#3B82F6]">Preview</div>
          ...
          <button type="button" onClick={addIngredient} className="inline-flex items-center justify-center gap-2 rounded-2xl border border-transparent vyron-grad-surface px-5 py-4 text-sm font-black text-[#F8FAFC] transition hover:bg-[#23304f]">
            <Plus size={18} />
            Add Ingredient
          </button>
            ...
            {filteredIngredients.map((item) => {
              ...
              return (
                <div key={item.id} className="grid grid-cols-8 items-center border-t border-slate-100 px-5 py-5 text-sm">
                  <div>
                    <Link href={`/ingredients/${item.id}`} className="font-black text-[#F8FAFC] hover:text-[#1D4ED8]">
                      {item.ingredient_name}
                    </Link>
                  ...
                  <div className="font-black text-[#1D4ED8]">{formatMoney(Number(item.true_unit_cost))}</div>
                  <div><StatusPill tone={tone}>{movement >= 0 ? "+" : ""}{movement.toFixed(1)}%</StatusPill></div>
                  <div>
                    <Link href={`/ingredients/${item.id}/edit`} className="inline-flex items-center gap-2 rounded-full border border-[#3B82F6]/25 bg-[#3B82F6]/10 px-3 py-2 text-xs font-black text-[#1D4ED8]">
                      <Edit3 size={14} />
                      Edit
                    </Link>
                  </div>
