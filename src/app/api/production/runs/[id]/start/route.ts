...
export async function POST(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    await requireWorkspacePermission("manufacturing.runs.start");
    const companyId = await requireManufacturingCompanyId(supabase, manufacturingCompanyContextFromRequest(request, body));
    const run = await transitionProductionRun(supabase, companyId, id, "start", String(body.actor || "user"));
    return NextResponse.json({ ok: true, run });
  } catch (error) {
    return workspaceAccessErrorResponse(error, "Start failed.");
  }
}
