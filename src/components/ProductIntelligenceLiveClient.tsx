  ...
  return (
    ...
    >
      <section className="grid gap-6">
        ...
        <div className="rounded-[2rem] border border-[#2C5A6B]/20 bg-[#2C5A6B]/10 p-5 shadow-sm">
          <div className="text-[10px] font-black uppercase tracking-[0.12em] text-[#163A48]">Healthy Margins</div>
          <div className="mt-2 text-3xl font-black text-[#2F7C40]">{products.length - erosionCount}</div>
        </div>
      </div>
          ...
          {products.length === 0 ? (
            ...
            products.map((row) => (
              ...
              >
                ...
                <div className={`font-black ${row.marginErosion ? "text-red-700" : "text-[#163A48]"}`}>
                  {row.gpPct.toFixed(1)}%
                </div>
