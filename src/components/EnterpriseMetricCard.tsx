  ...
  return (
    <VyronSurfaceCard elevated={dark} className="p-6">
      <div className="flex items-start justify-between gap-4">
        <div className={`rounded-2xl p-3 ${dark ? "vyron-grad-deep text-[#BCCDD5]" : "bg-[#F1F5F9] text-[#64748B]"}`}>
          <Icon size={23} />
        </div>
        <div className="rounded-full bg-[#EEF8F0] px-3 py-1 text-[10px] font-black text-[#1F4757]">LIVE</div>
      </div>
      <div className="mt-5 text-sm font-black uppercase tracking-[0.18em] text-[#64748B]">{title}</div>
      <div className="mt-2 text-3xl font-black tracking-tight text-[#0B202B]">{value}</div>
