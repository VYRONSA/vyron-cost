...
export const ISSUE_CATALOG: Record<string, IssueDefinition> = Object.fromEntries(
  [
    ...
    def({ code: "PRICE_MISMATCH", title: "Price differs from VYRON's price", category: "price", severity: "warning", source: "core", action: "Confirm the order price, or correct it to the price list." }),
    def({ code: "PRICE_FROM_VYRON", title: "Price taken from VYRON", category: "price", severity: "info", source: "core", action: "None — the order had no price, so the customer or standard price is used." }),
    ...
    def({ code: "MARGIN_NOT_MEASURED", title: "Margin not measured", category: "margin", severity: "info", source: "core", action: "None — some products have no cost in VYRON." }),
