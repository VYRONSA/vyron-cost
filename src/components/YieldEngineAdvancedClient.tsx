    ...
    <section className="grid gap-6 xl:grid-cols-[0.9fr_1.2fr]">
      <div className="rounded-[2rem] border border-white bg-white p-6 shadow-[0_10px_40px_rgba(15,23,42,0.06)]">
        <div className="mb-5 flex items-center gap-3">
          ...
          <div>
            <h2 className="text-2xl font-black text-[#07110d]">Advanced Yield Calculator</h2>
        ...
        <div className="grid gap-4">
          <label className="text-sm font-black text-slate-600">
            ...
            <select
              ...
              className="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-3 font-bold outline-none focus:border-emerald-400"
            >
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <label className="text-sm font-black text-slate-600">
              Raw Weight
              <input type="number" value={rawWeight} onChange={(event) => setRawWeight(event.target.value)} className="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-3 font-bold outline-none focus:border-emerald-400" />
            </label>

            <label className="text-sm font-black text-slate-600">
              Final Usable Weight
              <input type="number" value={finalWeight} onChange={(event) => setFinalWeight(event.target.value)} className="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-3 font-bold outline-none focus:border-emerald-400" />
            </label>
          </div>

          <label className="text-sm font-black text-slate-600">
            Raw Batch Cost
            <input type="number" value={rawCost} onChange={(event) => setRawCost(event.target.value)} className="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-3 font-bold outline-none focus:border-emerald-400" />
          </label>

          <button type="button" className="inline-flex items-center justify-center gap-2 rounded-2xl bg-emerald-500 px-5 py-4 text-sm font-black text-[#07110d] transition hover:bg-emerald-400">
            <Save size={18} />
      ...
      <div className="rounded-[2rem] bg-[#07110d] p-7 text-white shadow-[0_18px_55px_rgba(6,20,14,0.24)]">
        ...
        <div className="mt-8 grid gap-4 md:grid-cols-2">
          <div className="rounded-[1.5rem] border border-emerald-400/15 bg-white/5 p-5">
            <div className="text-xs font-black uppercase tracking-[0.18em] text-emerald-300">Ingredient</div>
            <div className="mt-2 text-2xl font-black">{selected?.ingredient_name || "None"}</div>
          </div>

          <div className="rounded-[1.5rem] border border-emerald-400/15 bg-white/5 p-5">
            <div className="text-xs font-black uppercase tracking-[0.18em] text-emerald-300">Yield Mode</div>
            <div className="mt-2"><StatusPill tone="emerald">{mode.replaceAll("_", " ")}</StatusPill></div>
          </div>

          <div className="rounded-[1.5rem] border border-emerald-400/15 bg-white/5 p-5">
            <div className="text-xs font-black uppercase tracking-[0.18em] text-emerald-300">Yield %</div>
            <div className="mt-2 text-4xl font-black">{yieldPercent.toFixed(1)}%</div>
          </div>

          <div className="rounded-[1.5rem] border border-emerald-400/15 bg-white/5 p-5">
            <div className="text-xs font-black uppercase tracking-[0.18em] text-emerald-300">True Usable Cost</div>
            <div className="mt-2 text-4xl font-black">{formatMoney(trueCost)}</div>
          </div>
        </div>

        <div className="mt-6 rounded-[1.5rem] border border-emerald-400/15 bg-emerald-400/10 p-5 text-sm leading-7 text-slate-200">
