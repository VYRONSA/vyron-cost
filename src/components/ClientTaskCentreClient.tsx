    ...
    <section className="grid gap-6">
      <div className="rounded-[2rem] bg-white p-6 shadow-[0_10px_40px_rgba(15,23,42,0.06)]">
        <div className="flex gap-3">
          ...
          <button onClick={addTask} className="inline-flex items-center gap-2 rounded-2xl bg-emerald-500 px-5 py-3 font-black text-[#07110d]"><Plus size={17} /> Add</button>
        </div>
      </div>
        ...
        {tasks.map(([task, area, priority]) => {
          ...
          return (
            <div key={task} className="grid gap-4 rounded-[2rem] bg-white p-5 shadow-[0_10px_40px_rgba(15,23,42,0.06)] md:grid-cols-[60px_1fr_140px_120px] md:items-center">
              <button onClick={() => setDone((current) => ({ ...current, [task]: !checked }))} className={`flex h-12 w-12 items-center justify-center rounded-2xl ${checked ? "bg-emerald-500 text-white" : "bg-slate-100 text-slate-400"}`}>
                <CheckCircle2 size={24} />
              </button>
              <div>
                <div className="font-black text-[#07110d]">{task}</div>
                <div className="text-xs font-bold text-slate-500">{area}</div>
              </div>
              <div className="font-black text-amber-700">{priority}</div>
              <div className={checked ? "font-black text-emerald-700" : "font-black text-slate-500"}>{checked ? "Done" : "Open"}</div>
            </div>
          );
        })}
      </div>
