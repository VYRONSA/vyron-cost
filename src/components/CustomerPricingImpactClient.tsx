  ...
  return (
    <section className="grid gap-6">
      <div className="rounded-[2rem] bg-[#07110d] p-6 text-white">
        ...
        <label className="mt-5 block max-w-sm text-sm font-black text-emerald-300">
          Price increase %
      ...
      <section className="grid gap-5 md:grid-cols-2">
        <div className="rounded-[2rem] bg-emerald-50 p-6">
          <div className="text-xs font-black uppercase tracking-[0.16em] text-emerald-700">Monthly Revenue Impact</div>
          <div className="mt-3 text-4xl font-black text-emerald-700">{money(monthly)}</div>
        </div>
        <div className="rounded-[2rem] bg-white p-6">
          <div className="text-xs font-black uppercase tracking-[0.16em] text-slate-400">Annualised</div>
          <div className="mt-3 text-4xl font-black text-[#07110d]">{money(monthly * 12)}</div>
        </div>
      </section>

      <div className="overflow-hidden rounded-[2rem] bg-white">
        <div className="grid grid-cols-5 bg-[#07110d] px-5 py-4 text-xs font-black uppercase text-emerald-300">
        ...
        {rows.map((row) => (
          <div key={row.product.id} className="grid grid-cols-5 border-t border-slate-100 px-5 py-5 text-sm">
            ...
            <div className="font-black text-emerald-700">{money(row.newPrice)}</div>
