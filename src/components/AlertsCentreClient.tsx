...
export default function AlertsCentreClient({
  ...
  return (
    <VyronPremiumPageShell
      config={{
        title: "Alerts Centre",
        subtitle: "Premium VOLORA workflow for alerts centre.",
      ...
      <section className="grid gap-6">
            ...
            <div className="grid gap-4">
              {filtered.map((alert) => (
                <div
                  key={alert.id}
                  className={`rounded-[2rem] border p-6 shadow-[0_10px_35px_rgba(11,32,43,0.06)] ${
                    alert.is_read
                      ? "border-white bg-white"
                      : "border-[#2C5A6B]/25 bg-white shadow-black/10"
                  }`}
                >
                  <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    ...
                    <button
                      ...
                      className="inline-flex items-center justify-center gap-2 rounded-2xl border border-transparent vyron-grad-surface px-5 py-3 text-sm font-black text-[#F8FAFC] transition hover:bg-[#163a48]"
                    >
