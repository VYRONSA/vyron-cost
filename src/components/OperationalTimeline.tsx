  ...
  return (
    <section className="rounded-[2rem] bg-[#07110d] p-6 text-white shadow-[0_20px_60px_rgba(6,20,14,0.28)]">
      <div className="flex items-center justify-between">
        <div>
          <div className="text-xs font-black uppercase tracking-[0.22em] text-[#A855F7]">LIVE OPERATIONS</div>
          <h2 className="mt-2 text-2xl font-black">Operational Timeline</h2>
        </div>
        <div className="rounded-full bg-[#A855F7]/12 px-4 py-2 text-xs font-black text-[#A855F7]">LIVE</div>
      </div>

      <div className="mt-6 space-y-4">
        {items.map((item) => (
          <div key={item.title} className="rounded-[1.5rem] border border-[#A855F7]/15 bg-white/5 p-5">
            <div className="flex items-start justify-between gap-4">
              ...
              <div className="text-xs font-black uppercase tracking-[0.18em] text-[#A855F7]">{item.time}</div>
            </div>
          </div>
        ))}
      </div>
