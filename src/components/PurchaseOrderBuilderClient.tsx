...
export default function PurchaseOrderBuilderClient({ suppliers, ingredients }: { suppliers: CostSupplier[]; ingredients: CostIngredient[] }) {
 ...
 <div className="grid gap-5 md:grid-cols-3"><div className="rounded-[2rem] bg-white p-5"><b>Subtotal</b><div className="text-3xl font-black">{formatMoney(totals.subtotal)}</div></div><div className="rounded-[2rem] bg-white p-5"><b>VAT</b><div className="text-3xl font-black">{formatMoney(totals.vat)}</div></div><div className="rounded-[2rem] bg-emerald-50 p-5"><b>Total</b><div className="text-3xl font-black text-emerald-600">{formatMoney(totals.total)}</div></div></div>
 {msg&&<div className="rounded-2xl bg-emerald-50 p-4 font-bold text-emerald-700">{msg}</div>}{err&&<div className="rounded-2xl bg-red-50 p-4 font-bold text-red-700">{err}</div>}
