    ...
    <section className="grid gap-6">
      <section className="grid gap-5 md:grid-cols-3">
        ...
        <div className="rounded-[2rem] bg-emerald-50 p-6 shadow-[0_10px_40px_rgba(15,23,42,0.06)]">
          <div className="text-xs font-black uppercase tracking-[0.16em] text-emerald-700">Approved Monthly Recovery</div>
        ...
        <div className="rounded-[2rem] bg-[#07110d] p-6 text-white shadow-[0_18px_55px_rgba(6,20,14,0.24)]">
          <div className="text-xs font-black uppercase tracking-[0.16em] text-emerald-300">Annualised Approved</div>
        ...
        <div className="min-w-[1100px]">
          ...
          {rows.map((row) => (
            <div key={row.id} className="grid grid-cols-9 items-center border-t border-slate-100 px-5 py-5 text-sm">
              <div className="font-black text-emerald-700">{row.finding_type}</div>
              <div className="col-span-2">
                <div className="font-black text-[#07110d]">{row.title}</div>
                <div className="text-xs text-slate-500">{row.description}</div>
              </div>
              <div className="font-black text-emerald-700">{money(row.recovery)}</div>
              ...
              <div className="col-span-2 flex gap-2">
                <button onClick={() => setDecisions((current) => ({ ...current, [row.id]: "Approved" }))} className="inline-flex items-center gap-2 rounded-full bg-emerald-50 px-3 py-2 text-xs font-black text-emerald-700">
