...
export default function ManufacturingReportsPage() {
  return (
    <ModulePageShell eyebrow="Reports Centre" title="Manufacturing Reports" subtitle="Batch history, cost variances, wastage and finished goods production reports." actions={<><BackButton /><button className="rounded-full border border-slate-200 bg-white px-4 py-2 text-sm font-black">Print</button><button className="rounded-full bg-violet-700 px-4 py-2 text-sm font-black text-white">Export CSV</button><button disabled className="rounded-full border border-slate-200 bg-slate-50 px-4 py-2 text-sm font-black text-slate-400">PDF coming soon</button></>}>
